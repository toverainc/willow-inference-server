...
Note that (like Willow) Willow Inference Server is very early and advancing rapidly! Users are encouraged to contribute (hence the build). For the 1.0 release of Willow and WIS we will provide ready to deploy Docker containers.
...
As you can see the realtime multiple increases dramatically with longer speech segments. Note that these numbers will also vary depending slightly depending on broader system configuration - CPU, RAM, etc.
